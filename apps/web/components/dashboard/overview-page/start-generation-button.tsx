...
import { ArrowUpRight, ExternalLink, Loader2, Plus, Sparkles } from "lucide-react"
...
export function StartGenerationButton() {
  ...
  const [isFreeTierLimitReached, setIsFreeTierLimitReached] = useState(false)
  ...
  useEffect(() => {
    if (error instanceof APIError && error.status === 403) {
      ...
      setIsFreeTierLimitReached(true)
    }
  }, [error])
  ...
  const isNonLimitError =
    error && !(error instanceof APIError && error.status === 403)
  const errorMessages = getErrorMessages(errors)
  return (
    <>
      {/* Free-tier limit reached dialog */}
      <Dialog
        open={isFreeTierLimitReached}
        onOpenChange={setIsFreeTierLimitReached}
      >
        <DialogContent className="max-w-md">
            ...
            <DialogDescription asChild>
              <div className="space-y-3 text-sm text-muted-foreground">
                <p>
                  ...
                  . We limit free generations to help control costs.
                </p>
                <p>
                  Want unlimited generations? You can run Resumind yourself for
                  free — the full source code is on GitHub.
                </p>
                <a
          ...
          <div className="flex justify-end pt-2">
            <Button
              variant="outline"
              onClick={() => setIsFreeTierLimitReached(false)}
            >
              Close
            </Button>
          </div>
